...
def connection_driven_greedy_algoritme(spel: Kaart) -> None:
    ...
    schrijf_output(schrijf_output_verbindingen, schrijf_output_trajecten, aantal_treinen, tijd_gereden, aantal_connecties_gereden, score, aantal_treinen)
...
def genereer_lijnvoering(spel: Kaart) -> tuple[list, list]:
    ...
    while trein1.time_driven <= 120:
        ...
        while (trein1.time_driven + reisduur > 120 or connection_visited) and counter < max:
        ...
        if not trein1.time_driven + reisduur > 120:
            trein1.traject_history.push(trein1.current_station.name)
            trein1.current_station.set_visited()
